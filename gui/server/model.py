...
def load_video(path, max_frame_no=78, frame_interval=2,device='cpu'):
    ...
    cap.release()
    if len(video_arr)<max_frame_no:
            empty_frame = torch.zeros((3,256,256)).to(device)
            padding  = [empty_frame]*(max_frame_no-len(video_arr))
            video_arr+=padding
...
df = pd.read_csv("./train_labels.csv", names = ["file_name", "label"])

# def get_label(filename):
#     print(filename[:-10])
#     row = df.loc[df['file_name'] == filename]
#     return row['label']

def label_to_text(idx):
    labeldf = pd.read_csv("./label_to_text.csv", names = ["ClassId", "TR", "EN"])
    row = labeldf.loc[labeldf['ClassId'] == f'{idx}']
    return row['EN'].values.item()
...
#     filename = f'signer0_sample{i}_color.mp4'
#     videotensor = load_video(filename)
#     print('video loaded')
#     output = model(videotensor.unsqueeze(0))
#     ps = torch.exp(output)
#     predictions = ps.max(dim=1)[1]
#     print(predictions)
    # label = get_label(filename)
    # idx = int(predictions)
    # text = label_to_text(idx)
    # print(idx, text)

def video_to_word(filename, model):
    videotensor = load_video(filename)
    print('video loaded')
    output = model(videotensor.unsqueeze(0))
    ps = torch.exp(output)
    predictions = ps.max(dim=1)[1]
    print(predictions)
    return label_to_text(predictions[0])
